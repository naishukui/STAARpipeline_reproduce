coding_combine <- function(chr,gene_name,genofile,obj_nullmodel,genes,
  ...
  variant.id.gene <- variant.id.gene[coding]
  seqSetFilter(genofile,variant.id=variant.id.gene,sample.id=phenotype.id)
    ...
    if(Use_annotation_weights)
    {
      for(k in 1:length(Annotation_name))
      {
        if(Annotation_name[k]%in%Annotation_name_catalog$name)
        {
          ...
          #kui_add this line to impute missing values for indels
          Annotation.PHRED[is.na(Annotation.PHRED)] <- min(Annotation.PHRED,na.rm=TRUE)
  ...
  if(class(pvalues)=="list")
  {
    ...
    results_combine <- rbind(results_combine,results_temp)
  }

  if(!is.null(results_combine))
  {
    colnames(results_combine) <- colnames(results_combine, do.NULL = FALSE, prefix = "col")
    colnames(results_combine)[1:5] <- c("Gene name","Chr","Category","#SNV","cMAC")
    colnames(results_combine)[(dim(results_combine)[2]-1):dim(results_combine)[2]] <- c("ACAT-O","STAAR-O")
  }
  ...
  results_coding <- list(combine=results_combine)
